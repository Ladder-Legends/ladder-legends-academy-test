...
export function VideoLibraryContent() {
  ...
  // Get active and inactive coach IDs
  const allCoachesData = coachesData as Coach[];
  const activeCoachIds = allCoachesData.filter(c => c.isActive !== false).map(c => c.id);
  const inactiveCoachIds = allCoachesData.filter(c => c.isActive === false).map(c => c.id);

  // Initialize state from URL parameters, filtering out inactive coaches for non-owners
  const [selectedItems, setSelectedItems] = useState<Record<string, string[]>>(() => {
    const coachParams = searchParams.get('coach')?.split(',').filter(Boolean) || [];
    // Filter out inactive coaches unless user is owner
    const filteredCoaches = userIsOwner
      ? coachParams
      : coachParams.filter(id => activeCoachIds.includes(id));
  ...
  const filters = useMemo(() => ({
    ...
    tags: selectedTags,
  }), [searchQuery, selectedItems.races, selectedItems.general, selectedItems.coaches, selectedItems.contentType, selectedItems.accessLevel, selectedTags]);
